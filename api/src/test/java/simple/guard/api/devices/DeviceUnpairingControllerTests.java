...
class DeviceUnpairingControllerTests {
    ...
    private static final UUID NON_ADMIN_ACCOUNT_ID = UUID.fromString("00000000-0000-0000-0000-000000000703");
    ...
    void revokesActiveKeysAndUpdatesDeviceStateTests() throws Exception {
        mockMvc.perform(delete("/api/devices/{deviceId}/unpairing", DEVICE_ID)
    ...
    void listsPendingUnpairingRequestsTests() throws Exception {
        ...
        mockMvc.perform(get("/api/devices/unpairing-requests")
                        .header("Authorization", "Bearer valid-token"))
                .andExpect(status().isOk())
    ...
    void approvesPendingUnpairingRequestTests() throws Exception {
        ...
        mockMvc.perform(post("/api/devices/unpairing-requests/{requestId}/decision", request.getId())
                        .header("Authorization", "Bearer valid-token")
        ...
        assertThat(unpairingRequests.findById(request.getId())).get().satisfies(currentRequest -> {
            assertThat(currentRequest.getStatus()).isEqualTo(DeviceUnpairingRequestStatus.APPROVED);
    ...
    void rejectsPendingUnpairingRequestTests() throws Exception {
        ...
        assertThat(unpairingRequests.findById(request.getId())).get().satisfies(currentRequest -> {
            assertThat(currentRequest.getStatus()).isEqualTo(DeviceUnpairingRequestStatus.REJECTED);
            assertThat(currentRequest.getDecidedBy()).isEqualTo(ADMINISTRATOR_SUBJECT);
        });
        assertThat(devices.findById(DEVICE_ID)).get()
                .satisfies(device -> assertThat(device.getPairingStatus()).isEqualTo(DevicePairingStatus.PAIRED));
        assertThat(deviceKeys.findAll()).singleElement()
    ...
    void rejectsPendingStatusAsUnpairingRequestDecisionTests() throws Exception {
        ...
        assertThat(unpairingRequests.findById(request.getId())).get()
                .satisfies(currentRequest -> assertThat(currentRequest.getStatus()).isEqualTo(DeviceUnpairingRequestStatus.PENDING));
        assertThat(deviceKeys.findAll()).singleElement()
                .satisfies(key -> assertThat(key.getStatus()).isEqualTo(DeviceKeyStatus.ACTIVE));
    }
    ...
    void rejectsUnknownPendingUnpairingRequestDecisionTests() throws Exception {
        mockMvc.perform(post("/api/devices/unpairing-requests/{requestId}/decision", UUID.randomUUID())
                        .header("Authorization", "Bearer valid-token")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {
    ...
    static class JwtDecoderTestConfigurationTests {
        ...
        JwtDecoder jwtDecoderTests() {
            return token -> {
                String subject = "non-admin-token".equals(token) ? NON_ADMINISTRATOR_SUBJECT : ADMINISTRATOR_SUBJECT;
                ...
                return new Jwt(
                        ...
                        Map.of(
                                ...
                                "sub", subject,
                                "email", subject + "@simpleguard.local"
                        )
                );
            };
        }
