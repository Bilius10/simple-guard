...
class DeviceControllerTests {
  ...
  void createsValidUnpairedDeviceTests() throws Exception {
    mockMvc
        ...
        .andExpect(
            header().string("Location", org.hamcrest.Matchers.matchesPattern("/api/devices/.+")))
  ...
  void listsRegisteredDevicesTests() throws Exception {
    ...
    mockMvc
        .perform(get("/api/devices").header("Authorization", "Bearer valid-token"))
  ...
  void generatesAuditableShortPairingSessionTests() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/devices/{deviceId}/pairing-sessions", device.getId())
                .header("Authorization", "Bearer valid-token"))
        .andExpect(status().isCreated())
        .andExpect(
            header()
        ...
        .andExpect(
            jsonPath("$.pairingCode")
                .value(org.hamcrest.Matchers.matchesPattern("[A-Z2-9]{4}-[A-Z2-9]{4}")))
  ...
  void rejectsNewPairingSessionWhenValidWaitingSessionExistsTests() throws Exception {
    ...
    mockMvc
        ...
        .andExpect(jsonPath("$.uri").value("/api/devices/" + device.getId() + "/pairing-sessions"));

    assertThat(pairingSessions.findAll())
        .singleElement()
        .satisfies(
            session -> assertThat(session.getStatus()).isEqualTo(PairingSessionStatus.WAITING));
  }
    ...
    assertThat(pairingSessions.findAll())
        ...
        .anySatisfy(
            session -> {
              ...
              assertThat(session.getExpirationReason())
                  .isEqualTo(PairingSessionExpirationReason.TIMEOUT);
  ...
  void rejectsPairingSessionForUnknownDeviceTests() throws Exception {
    ...
    mockMvc
        .perform(
            post("/api/devices/{deviceId}/pairing-sessions", unknownDeviceId)
                .header("Authorization", "Bearer valid-token"))
  ...
  private void generatePairingSessionTests(UUID deviceId) throws Exception {
    mockMvc
        .perform(
            post("/api/devices/{deviceId}/pairing-sessions", deviceId)
                .header("Authorization", "Bearer valid-token"))
        .andExpect(status().isCreated());
  }
