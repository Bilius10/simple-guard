...
class AgentUnpairingControllerTests {
  ...
  @Autowired private DeviceUnpairingRequestService unpairingRequestService;
  ...
  void requestsUnpairingWithValidProofOfPossessionTests() throws Exception {
    mockMvc
        .perform(
            delete("/api/agent/devices/{deviceId}/pairing", DEVICE_ID)
    ...
    assertThat(unpairingRequests.findAll())
        .singleElement()
        .satisfies(
            request -> {
  ...
  void reportsPendingAndApprovedUnpairingToAgentTests() throws Exception {
    ...
    mockMvc
        .perform(
            get("/api/agent/devices/{deviceId}/pairing", DEVICE_ID)
                .header("X-Agent-Instance-Id", AGENT_INSTANCE_ID)
                .header("X-Agent-Signature", signature))
    ...
    mockMvc
        .perform(
            get("/api/agent/devices/{deviceId}/pairing", DEVICE_ID)
                .header("X-Agent-Instance-Id", AGENT_INSTANCE_ID)
                .header(
                    "X-Agent-Signature", Base64.getEncoder().encodeToString("invalid".getBytes())))
