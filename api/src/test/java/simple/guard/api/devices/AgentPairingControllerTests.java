...
class AgentPairingControllerTests {
  ...
  @Autowired private PairingCodeHasher codeHasher;
  ...
  void completesAgentPairingWithValidCodeAndPublicKeyTests() throws Exception {
    ...
    assertThat(pairingSessions.findById(session.getId()))
        .get()
        .satisfies(
            usedSession -> {
  ...
  void rejectsAgentPairingWithExpiredCodeTests() throws Exception {
    PairingSession session =
        pairingSessions.save(waitingSessionTests(OffsetDateTime.now().minusSeconds(1)));
  ...
  private static Device deviceTests(
      DevicePairingStatus status, DeviceType type, DevicePlatform platform, String name) {
    return new Device(
        DEVICE_ID,
        ACCOUNT_ID,
        name,
  ...
  private static String pairingRequestTests(
      String pairingCode, String agentInstanceId, String publicKey) {
    return pairingRequestTests(pairingCode, agentInstanceId, DevicePlatform.ANDROID, publicKey);
  }

  private static String pairingRequestTests(
      String pairingCode, String agentInstanceId, DevicePlatform platform, String publicKey) {
    return """
                {
                  "pairingCode": "%s",
                  "agentInstanceId": "%s",
