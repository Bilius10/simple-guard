...
public class DeviceKeyService {
  ...
  public DeviceKey requireByDeviceIdAndAgentInstanceIdAndStatus(
    ...
    return deviceKeys
        .findByDeviceIdAndAgentInstanceIdAndStatus(deviceId, agentInstanceId, deviceKeyStatus)
        .orElseThrow(
            () -> new SimpleGuardException(HttpStatus.UNAUTHORIZED, errorCode, translation));
  }

  public DeviceKey requireForAgentPairingStatus(UUID deviceId, String agentInstanceId) {
    return deviceKeys
        .findFirstByDeviceIdAndAgentInstanceIdOrderByCreatedAtDesc(deviceId, agentInstanceId)
        .orElseThrow(
            () ->
  ...
  public void registerActiveKey(
      Device device, UUID pairingSessionId, String agentInstanceId, String publicKey) {
    deviceKeys.saveAndFlush(
        DeviceKey.active(
            UUID.randomUUID(),
            device.getId(),
            pairingSessionId,
  ...
  public int revokeActiveKeysForDevice(UUID deviceId, String actor, OffsetDateTime revokedAt) {
    List<DeviceKey> activeKeys =
        deviceKeys.findAllByDeviceIdAndStatus(deviceId, DeviceKeyStatus.ACTIVE);
    int revokedKeyCount =
        (int) activeKeys.stream().filter(key -> key.revoke(actor, revokedAt)).count();
