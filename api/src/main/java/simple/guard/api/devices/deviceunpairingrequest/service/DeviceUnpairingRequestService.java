...
public class DeviceUnpairingRequestService {
    ...
    private DeviceUnpairingRequestResponse createPendingRequest(UUID deviceId, String agentInstanceId) {
        ...
        DeviceUnpairingRequest request = requests.findFirstByDeviceIdAndStatusOrderByRequestedAtDesc(
                        deviceId,
                        DeviceUnpairingRequestStatus.PENDING
                )
                .orElseGet(() -> requests.saveAndFlush(
    ...
    public List<DeviceUnpairingRequestResponse> listPending(Account account) {
        return requests.findAllByAccountIdAndStatusOrderByRequestedAtDesc(
                        account.getId(),
                        DeviceUnpairingRequestStatus.PENDING
                )
                .stream()
                .map(request -> DeviceUnpairingRequestResponse.from(
                        request,
    ...
    @Transactional
    public DeviceUnpairingDecisionResponse decide(UUID requestId, Account account, DeviceUnpairingRequestStatus decision) {
        ...
        return new DeviceUnpairingDecisionResponse(
                DeviceUnpairingRequestResponse.from(request, device),
                unpairing
        );
    }

    private DeviceUnpairingRequest findPendingRequest(UUID requestId, Account account) {
        return requests.findByIdAndAccountIdAndStatus(
                        requestId,
                        account.getId(),
                        DeviceUnpairingRequestStatus.PENDING
                )
