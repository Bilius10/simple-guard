...
public class DeviceTelemetryBatchService {
    ...
    public DeviceTelemetryBatchResponse ingest(
            ...
            CreateDeviceTelemetryBatchRequest request
    ) {
        ...
        for (int index = 0; index < request.events().size(); index++) {
            results.add(ingestItem(deviceKey, deviceId, agentInstanceId, index, request.events().get(index)));
        }
    ...
    private DeviceTelemetryBatchItemResponse ingestItem(
        ...
        DeviceTelemetryBatchItemResponse requiredTelemetryError = validateRequiredTelemetry(index, item);
        ...
        DeviceTelemetryBatchItemResponse signatureError = validateSignature(
                deviceKey,
                deviceId,
                agentInstanceId,
                index,
        ...
        DeviceTelemetryResponse response = telemetry.ingest(
                deviceId,
                agentInstanceId,
                item.signature(),
                item.telemetry()
        ...
        return new DeviceTelemetryBatchItemResponse(
                ...
                response.duplicate() ? DeviceTelemetryBatchItemStatus.DUPLICATE : DeviceTelemetryBatchItemStatus.ACCEPTED,
                null
        );
    }
    ...
    private DeviceTelemetryBatchItemResponse validateSignature(
        ...
        if (item.signature() == null || item.signature().isBlank() || !signatureVerifier.verifyTelemetry(
