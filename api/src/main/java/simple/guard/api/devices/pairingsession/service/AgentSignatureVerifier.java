...
public class AgentSignatureVerifier {
  ...
  public boolean verifyUnpairing(
      String encodedPublicKey, UUID deviceId, String agentInstanceId, String encodedSignature) {
  ...
  public static byte[] unpairingPayload(UUID deviceId, String agentInstanceId) {
    return ("UNPAIR_DEVICE\n" + deviceId + "\n" + agentInstanceId).getBytes(StandardCharsets.UTF_8);
  }
  ...
  public boolean verifyTelemetry(
    ...
    return verify(
        encodedPublicKey, telemetryPayload(deviceId, agentInstanceId, request), encodedSignature);
  }

  public static byte[] telemetryPayload(
      UUID deviceId, String agentInstanceId, CreateDeviceTelemetryRequest request) {
    ...
    return String.join(
            ...
            permissions == null || permissions.fineLocation() == null
                ? ""
                : permissions.fineLocation(),
            permissions == null || permissions.coarseLocation() == null
                ? ""
