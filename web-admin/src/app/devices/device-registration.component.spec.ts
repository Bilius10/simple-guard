...
describe('DeviceRegistrationComponentTests', () => {
  ...
  beforeEach(async () => {
    ...
    deviceApiStub.generatePairingSession.mockReturnValue(
      of(pairingSessionTests()),
    );
    deviceApiStub.listUnpairingRequests.mockReturnValue(of([]));
    deviceApiStub.unpair.mockReturnValue(
      of({
    ...
    deviceApiStub.decideUnpairingRequest.mockReturnValue(
      of({
        request: {
  ...
  it('registersAndListsDeviceAsPendingPairingTests', async () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(element.querySelector('.device-name')?.textContent).toContain(
      'Celular operacional',
    );
    expect(element.querySelector('.pairing-status')?.textContent).toContain(
    ...
    expect(notificationStub.success).toHaveBeenCalledWith(
      'Celular operacional cadastrado e pendente de pareamento.',
    );
  });
  ...
  it('showsLoadFailureAndRetryActionTests', async () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(element.querySelector('.error-state')?.textContent).toContain(
  ...
  it('showsBackendMessageWhenDeviceRequestFailsTests', async () => {
    deviceApiStub.list.mockReturnValue(
      throwError(() => ({
        error: {
  ...
  it('keepsFormDataWhenRegistrationFailsTests', async () => {
    deviceApiStub.create.mockReturnValue(
  ...
  it('showsWaitingPairingSessionTests', async () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(deviceApiStub.generatePairingSession).toHaveBeenCalledWith(
      androidDevice.deviceId,
    );
    const dialog = element.querySelector<HTMLElement>(
      '.pairing-console[role="dialog"]',
    );
    expect(dialog?.textContent).toContain('Aguardando agente');
    expect(dialog?.closest('.device-list')).toBeNull();
    expect(element.querySelector('.pairing-code')?.textContent).toContain(
      'ABCD-2345',
    );
    expect(element.querySelector('.pairing-expiration')?.textContent).toContain(
      'Expira em',
    );
  });
  ...
  it('showsExpiredPairingSessionTests', async () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(element.querySelector('.expired-state')?.textContent).toContain(
      'Codigo expirado',
    );
    expect(element.querySelector('.regenerate-action')?.textContent).toContain(
      'Gerar novo codigo',
    );
  ...
  it('closesActivePairingSessionTests', async () => {
    ...
    expect(
      (fixture.nativeElement as HTMLElement).querySelector('.waiting-state'),
    ).toBeNull();
  });
  ...
  it('returnsRawLabelsForUnknownDeviceValuesTests', () => {
    ...
    expect(fixture.componentInstance.typeLabel('UNKNOWN' as never)).toBe(
      'UNKNOWN',
    );
    expect(fixture.componentInstance.platformLabel('UNKNOWN' as never)).toBe(
      'UNKNOWN',
    );
  });
  ...
  it('unpairsDeviceAndUpdatesListAfterConfirmationTests', async () => {
    deviceApiStub.list.mockReturnValue(
      of([pairedAndroidDevice, pairedNotebookDevice]),
    );
    deviceApiStub.listUnpairingRequests.mockReturnValue(
      of([
        unpairingRequestTests(),
        {
          ...unpairingRequestTests(),
    ...
    expect(deviceApiStub.unpair).toHaveBeenCalledWith(
      pairedAndroidDevice.deviceId,
    );
    expect(fixture.componentInstance.devices()[0].pairingStatus).toBe(
      'unpaired',
    );
    expect(fixture.componentInstance.devices()[1]).toEqual(
      pairedNotebookDevice,
    );
  ...
  it('keepsDangerousConfirmationOpenWhenUnpairingFailsTests', async () => {
    ...
    fixture.componentInstance.openUnpairConfirmation(pairedAndroidDevice);
    await fixture.componentInstance.confirmUnpair(
      fixture.componentInstance.unpairAction()!,
    );
    fixture.detectChanges();

    expect(fixture.componentInstance.devices()[0].pairingStatus).toBe('paired');
    expect(fixture.componentInstance.unpairError()).toContain(
    ...
    expect(notificationStub.error).toHaveBeenCalledWith(
      'Nao foi possivel desparear Celular operacional.',
    );
  });
  ...
  it('showsPendingUnpairingRequestActionsTests', async () => {
    ...
    const element = fixture.nativeElement as HTMLElement;
    expect(
      element.querySelector('.unpairing-request-status')?.textContent,
    ).toContain('Despareamento solicitado');
    expect(
      element.querySelector('.approve-unpair-request-action'),
    ).not.toBeNull();
    expect(
      element.querySelector('.reject-unpair-request-action'),
    ).not.toBeNull();
  ...
  it('approvesPendingUnpairingRequestAndUpdatesDeviceTests', async () => {
    ...
    fixture.componentInstance.openApproveUnpairingRequestConfirmation(
      unpairingRequestTests(),
    );
    ...
    expect(deviceApiStub.decideUnpairingRequest).toHaveBeenCalledWith(
      'request-001',
      'approved',
    );
    expect(fixture.componentInstance.devices()[0].pairingStatus).toBe(
  ...
  it('removesApprovedRequestWithoutDeviceUpdateWhenNoUnpairingResponseTests', async () => {
    ...
    await fixture.componentInstance.confirmApproveUnpairingRequest(
      fixture.componentInstance.unpairingRequestAction()!,
    );
  ...
  it('keepsApprovalConfirmationOpenWhenDecisionFailsTests', async () => {
    ...
    expect(fixture.componentInstance.devices()[0].pairingStatus).toBe('paired');
    expect(fixture.componentInstance.unpairingRequestError()).toContain(
      'Nenhuma alteracao local',
    );
  ...
  it('rejectsPendingUnpairingRequestTests', async () => {
    ...
    await fixture.componentInstance.rejectUnpairingRequest(
      unpairingRequestTests(),
    );
    fixture.detectChanges();
