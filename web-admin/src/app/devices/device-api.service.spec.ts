...
import { CreateDeviceRequest, Device, DeviceUnpairingRequest, PairingSession } from './device.models';
...
describe('DeviceApiServiceTests', () => {
  ...
  it('reusesPairingSessionFromSessionStorageForTwoMinutesTests', () => {
    ...
    service.generatePairingSession('device-001').subscribe(result => cachedResult = result);
  ...
  it('ignoresInvalidCachedPairingSessionTests', () => {
    sessionStorage.setItem('simpleguard.pairing-session.device-001', JSON.stringify({
    ...
    const request = http.expectOne('/api/devices/device-001/pairing-sessions');
    expect(request.request.method).toBe('POST');
    expect(sessionStorage.getItem('simpleguard.pairing-session.device-001')).toBeNull();
    request.flush(pairingSessionTests());
  });

  it('ignoresMalformedCachedPairingSessionTests', () => {
    sessionStorage.setItem('simpleguard.pairing-session.device-001', '{not-json');
  ...
  it('unpairsDeviceThroughApiTests', () => {
    ...
    expect(request.request.headers.has('X-SimpleGuard-Confirmation')).toBe(false);
    request.flush({
      deviceId: 'device-001',
  ...
  it('approvesPendingUnpairingRequestTests', () => {
    service.decideUnpairingRequest('request-001', 'approved').subscribe(result => {
      expect(result.request.status).toBe('approved');
      expect(result.unpairing?.pairingStatus).toBe('unpaired');
    });

    const request = http.expectOne('/api/devices/unpairing-requests/request-001/decision');
    expect(request.request.method).toBe('POST');
    expect(request.request.body).toEqual({
      status: 'approved',
    });
  ...
  it('rejectsPendingUnpairingRequestTests', () => {
    service.decideUnpairingRequest('request-001', 'rejected').subscribe(result => {
